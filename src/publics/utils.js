...
export const cookieCan = (name, value, options, noMd5) => {
  ...
  if (
    ...
    if (document.cookie.length > 0) {
      const key = noMd5 ? name : md5(name)
      const keyPosition = document.cookie.indexOf(key)
      if (keyPosition < 0) {
        return ''
      }

      cookiePosBegin = keyPosition + key.length + 1
...
export const pureCookie = (name, value, options) => cookieCan(name, value, options, true)
